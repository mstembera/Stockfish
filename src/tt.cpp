...
TTEntry* TranspositionTable::probe(const Key key, bool& found, uint8_t pvnode) const {
  ...
  for (int i = 0; i < ClusterSize; ++i)
      ...
      {
          if ((((tte[i].genBound8 & 0xF8) != generation8) || (pvnode && !(tte[i].genBound8 & 0x4))) && tte[i].key16)
              tte[i].genBound8 = uint8_t(generation8 | (pvnode << 2) | tte[i].bound()); // Refresh
