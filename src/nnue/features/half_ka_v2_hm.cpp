...
void HalfKAv2_hm::write_indices(const std::array<Piece, SQUARE_NB>& oldPieces,
    ...
    // PieceSquareIndex and KingBuckets entries are multiples of 64 while the
    // oriented square occupies only the low 6 bits, so
    // (s ^ orient) + psi[pc] + bucket == s ^ (psi[pc] + bucket + orient)
    // and the square orientation can be folded into the piece lookup.
    ...
    _mm512_storeu_si512(write_added,
                        _mm512_cvtepu16_epi32(_mm512_castsi512_si256(added_indices)));
