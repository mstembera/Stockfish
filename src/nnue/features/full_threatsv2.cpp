...
inline sf_always_inline IndexType FullThreatsv2::make_index(
  Color perspective, Piece attacker, Square from, Square to, Piece attacked, Square ksq) {
    ...
    const unsigned persp_neg         = -unsigned(perspective);
    const unsigned attacker_oriented = unsigned(attacker) ^ (persp_neg & 8);
    const unsigned attacked_oriented = unsigned(attacked) ^ (persp_neg & 8);
    ...
    const bool     swapFromTo    = ((attacker ^ attacked) == 8) & (type_of(attacker) != PAWN);
    const unsigned orientation   = OrientTBL[ksq] ^ (56 * perspective);
    const unsigned from_raw      = unsigned(from) ^ orientation;
    const unsigned to_raw        = unsigned(to)   ^ orientation;
    const unsigned from_oriented = swapFromTo ? to_raw   : from_raw;
    const unsigned to_oriented   = swapFromTo ? from_raw : to_raw;

    const auto& lut1 = index_lut1[attacker_oriented][attacked_oriented];
    const auto& lut2 = index_lut2[attacker_oriented][from_oriented];

    return lut1[from_raw < to_raw]
