  ...
  class AffineTransform {
   ...
   // Read network parameters
    bool ReadParameters(std::istream& stream) {
      ...
      // Determine if sextuplets of weight and input products can be summed using 16bits
      // without saturation. We assume worst case combinations of 0 and 127 for all inputs.
      if (!stream.fail())
      {
          auto can_saturate = [](const WeightType* w, const int idx[6]) {
              int pSum = 0, nSum = 0;
              for (int p = 0; p < 6; ++p)
                  if (w[idx[p]] > 0)
                      pSum += w[idx[p]];
          ...
          for (IndexType i = 0; i < kOutputDimensions; ++i)
          {
              ...
              for (IndexType j = 0; j < (kPaddedInputDimensions / 192 * 192) && !canSaturate16[i]; j += 192)
                  for (int k = 0; k < 64 && !canSaturate16[i]; k += 2)
                  {
                      constexpr int spacing[6] = { 0, 1, 64, 65, 128, 129 };
                      canSaturate16[i] = can_saturate(&w[j + k], spacing);
                  }
#elif defined (USE_AVX2)
              for (IndexType j = 0; j < (kPaddedInputDimensions / 96 * 96) && !canSaturate16[i]; j += 96)
                  for (int k = 0; k < 32 && !canSaturate16[i]; k += 2)
                  {
                      constexpr int spacing[6] = { 0, 1, 32, 33, 64, 65 };
                      canSaturate16[i] = can_saturate(&w[j + k], spacing);
                  }
#elif defined (USE_SSSE3)
              for (IndexType j = 0; j < (kPaddedInputDimensions / 48 * 48) && !canSaturate16[i]; j += 48)
                  for (int k = 0; k < 16 && !canSaturate16[i]; k += 2)
                  {
                      constexpr int spacing[6] = { 0, 1, 16, 17, 32, 33 };
                      canSaturate16[i] = can_saturate(&w[j + k], spacing);
                  }
#endif
          }
    ...
    const OutputType* Propagate(
      ...
      [[maybe_unused]] auto m512_add_dpbusd_epi32x3 = [=](__m512i& acc,
          __m512i a0, __m512i b0, __m512i a1, __m512i b1, __m512i a2, __m512i b2) {
#if defined (USE_VNNI)
      ...
      [[maybe_unused]] auto m256_add_dpbusd_epi32x3 = [=](__m256i& acc,
          __m256i a0, __m256i b0, __m256i a1, __m256i b1, __m256i a2, __m256i b2) {
#if defined (USE_VNNI)
      ...
      [[maybe_unused]] auto m128_add_dpbusd_epi32x3 = [=](__m128i& acc,
          __m128i a0, __m128i b0, __m128i a1, __m128i b1, __m128i a2, __m128i b2) {
      ...
      else if constexpr (kOutputDimensions % 4 == 0)
      {
        for (IndexType i = 0; i < kOutputDimensions; i += 4)
        {
          ...
          if constexpr (kPaddedInputDimensions % (kSimdWidth * 2) == 0)
          {
            ...
            {
                for (; j < (int)kNumChunks512 - 2; j += 3)
                {
                    const __m512i in0 = input_vector512[j];
                    const __m512i in1 = input_vector512[j + 1];
                    const __m512i in2 = input_vector512[j + 2];

                    m512_add_dpbusd_epi32x3(sum0, in0, row0[j], in1, row0[j + 1], in2, row0[j + 2]);
                    m512_add_dpbusd_epi32x3(sum1, in0, row1[j], in1, row1[j + 1], in2, row1[j + 2]);
                    m512_add_dpbusd_epi32x3(sum2, in0, row2[j], in1, row2[j + 1], in2, row2[j + 2]);
                    m512_add_dpbusd_epi32x3(sum3, in0, row3[j], in1, row3[j + 1], in2, row3[j + 2]);
                }
            }
      ...
      if constexpr (kOutputDimensions % 4 == 0)
      {
        for (IndexType i = 0; i < kOutputDimensions; i += 4)
        {
          ...
          if (!canSaturate16x4[i / 4])
          {
              for (; j < (int)kNumChunks - 2; j += 3)
              {
                  const __m256i in0 = input_vector[j];
                  const __m256i in1 = input_vector[j + 1];
                  const __m256i in2 = input_vector[j + 2];

                  m256_add_dpbusd_epi32x3(sum0, in0, row0[j], in1, row0[j + 1], in2, row0[j + 2]);
                  m256_add_dpbusd_epi32x3(sum1, in0, row1[j], in1, row1[j + 1], in2, row1[j + 2]);
                  m256_add_dpbusd_epi32x3(sum2, in0, row2[j], in1, row2[j + 1], in2, row2[j + 2]);
                  m256_add_dpbusd_epi32x3(sum3, in0, row3[j], in1, row3[j + 1], in2, row3[j + 2]);
              }
          }
        ...
        for (IndexType i = 0; i < kOutputDimensions; i += 4)
        {
          ...
          if (!canSaturate16x4[i / 4])
          {
              for (; j < (int)kNumChunks - 2; j += 3)
              {
                  ...
                  const __m128i in2 = input_vector[j + 2];

                  m128_add_dpbusd_epi32x3(sum0, in0, row0[j], in1, row0[j + 1], in2, row0[j + 2]);
                  m128_add_dpbusd_epi32x3(sum1, in0, row1[j], in1, row1[j + 1], in2, row1[j + 2]);
                  m128_add_dpbusd_epi32x3(sum2, in0, row2[j], in1, row2[j + 1], in2, row2[j + 2]);
                  m128_add_dpbusd_epi32x3(sum3, in0, row3[j], in1, row3[j + 1], in2, row3[j + 2]);
              }
          }
          for (; j < (int)kNumChunks; ++j)
          {
