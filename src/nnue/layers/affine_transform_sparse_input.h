...
alignas(CacheLineSize) static constexpr struct OffsetIndices {
    ...
    constexpr OffsetIndices() : offset_indices() {
        ...
        for (int i = 0; i < 256; ++i)
        {
            ...
            [[maybe_unused]] std::uint64_t popCnt = k;
            while (k < 8)
                offset_indices[i][k++] = 0;
        #if (USE_SSE41)
            offset_indices[i][7] |= popCnt << 3;
        #endif
        }
    }
...
void find_nnz(const std::int32_t* input, std::uint16_t* out, IndexType& count_out) {
    ...
    for (IndexType i = 0; i < NumChunks; ++i)
    {
        ...
        for (IndexType j = 0; j < OutputsPerChunk; ++j)
        {
            ...
            const vec128_t offsets = _mm_cvtepu8_epi16(_mm_cvtsi64_si128(offsets64 & 0x0707070707070707ULL));
