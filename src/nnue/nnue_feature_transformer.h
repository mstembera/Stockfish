  ...
  class FeatureTransformer {
   ...
   private:
    void update_accumulator(const Position& pos, const Color perspective) const {
      ...
      while (   stID < stN - 1
             && stl[stID]->previous
             && !stl[stID]->accumulator.computed[perspective])
      {
        // This governs when a full feature refresh is needed and how many
        // updates are better than just one full refresh.
        if (   FeatureSet::requires_refresh(stl[stID], perspective)
            || (gain -= FeatureSet::update_cost(stl[stID]) + 1) < 0)
          break;

        stl[stID + 1] = stl[stID]->previous;
        ++stID;
      }

      if (stl[stID]->accumulator.computed[perspective])
      {
        if (stID == 0)
          return;

        StateInfo* baseSt = stl[stID];
        --stID;
        ...
        for (IndexType j = 0; j < HalfDimensions / TileHeight; ++j)
        {
          ...
          auto accTile = reinterpret_cast<vec_t*>(
              &baseSt->accumulator.accumulation[perspective][j * TileHeight]);

          for (IndexType k = 0; k < NumRegs; ++k)
            acc[k] = vec_load(&accTile[k]);

          for (int i = stID; i >= 0; --i)
          {
            ...
            accTile = reinterpret_cast<vec_t*>(
              &stl[i]->accumulator.accumulation[perspective][j * TileHeight]);
            for (IndexType k = 0; k < NumRegs; ++k)
        ...
        for (IndexType j = 0; j < PSQTBuckets / PsqtTileHeight; ++j)
        {
          ...
          auto accTilePsqt = reinterpret_cast<psqt_vec_t*>(
              &baseSt->accumulator.psqtAccumulation[perspective][j * PsqtTileHeight]);
          ...
          for (int i = stID; i >= 0; --i)
          {
            ...
            accTilePsqt = reinterpret_cast<psqt_vec_t*>(
              &stl[i]->accumulator.psqtAccumulation[perspective][j * PsqtTileHeight]);
            for (std::size_t k = 0; k < NumPsqtRegs; ++k)
        ...
        for (int i = stID; i >= 0; --i)
        {
          std::memcpy(&stl[i]->accumulator.accumulation[perspective],
              baseSt->accumulator.accumulation[perspective],
              HalfDimensions * sizeof(BiasType));

          for (std::size_t k = 0; k < PSQTBuckets; ++k)
              stl[i]->accumulator.psqtAccumulation[perspective][k] = baseSt->accumulator.psqtAccumulation[perspective][k];
          ...
          for (const auto index : removed[i])
          {
            ...
            for (IndexType j = 0; j < HalfDimensions; ++j)
                baseSt->accumulator.accumulation[perspective][j] -= weights[offset + j];

            for (std::size_t k = 0; k < PSQTBuckets; ++k)
                baseSt->accumulator.psqtAccumulation[perspective][k] -= psqtWeights[index * PSQTBuckets + k];
          }
