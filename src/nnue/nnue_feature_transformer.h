    ...
    void update_accumulator_refresh_cache(const Position&                           pos,
        ...
        for (IndexType j = 0; j < HalfDimensions / TileHeight; ++j)
        {
            ...
            for (; i < int(std::min(removed.size(), added.size())) - 1; i+=2)
            {
                ...
                const IndexType offsetA0 = HalfDimensions * indexA1 + j * TileHeight;
                const IndexType offsetA1 = HalfDimensions * indexA0 + j * TileHeight;
                ...
                for (unsigned k = 0; k < NumRegs; ++k)
                    acc[k] = vec_add_16(acc[k], vec_sub_16(vec_add_16(columnA0[k], columnA1[k]), vec_add_16(columnR0[k], columnR1[k])));
            }
