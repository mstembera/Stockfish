...
#undef S
  ...
  Score Evaluation<T>::pieces() {
    ...
    constexpr Bitboard OutpostRanks2 = (Us == WHITE ? Rank7BB
                                                    : Rank2BB);
    ...
    for (Square s = *pl; s != SQ_NONE; s = *++pl)
    {
        ...
        if (Pt == BISHOP || Pt == KNIGHT)
        {
            ...
            bb = (OutpostRanks | OutpostRanks2) & attackedBy[Us][PAWN] & ~pe->pawn_attacks_span(Them);
            if (s & bb)
                score += Outpost * ((s & bb & OutpostRanks) ? 2 : 1) * (Pt == KNIGHT ? 2 : 1);

            else if (Pt == KNIGHT && bb & b & ~pos.pieces(Us))
                score += Outpost * ((bb & b & ~pos.pieces(Us) & OutpostRanks) ? 2 : 1);
