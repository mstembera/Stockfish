...
Value Eval::evaluate(const Position& pos) {
  ...
  else
  {
      ...
      Value psq = eg_value(pos.psq_score());
      int   r50 = 16 + pos.rule50_count();
      bool  largePsq = abs(psq) * 16 > (NNUEThreshold1 + pos.non_pawn_material() / 64) * r50;
      bool  classical = largePsq || (abs(psq) > PawnValueMg / 4 && !(pos.this_thread()->nodes & 0xB));
