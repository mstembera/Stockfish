...
#define S(v) int(v)
  ...
  // Polynomial material imbalance parameters
    ...
    //            OUR PIECES
    // pair pawn knight bishop rook queen knightPair
    { S(1438)                                        }, // Bishop pair
    { S(  40), S( 38)                                }, // Pawn
    { S(  32), S(255), S(-62)                        }, // Knight      OUR PIECES
    { S(   0), S(104), S(  4), S(  0)                }, // Bishop
    { S( -26), S( -2), S( 47), S(105), S(-208)       }, // Rook
    { S(-189), S( 24), S(117), S(133), S(-134), S(-6)}, // Queen
    { S(   0), S(  0), S(  0), S(  0), S(   0), S( 0), S(0)}  // Knight pair
  };
    ...
    { S(97), S(100), S(-42), S(137), S(268), S(0) }, // Queen
    { S( 0), S(  0), S(  0), S(  0), S(  0), S(0), S(0) }  // Knight pair
  };
...
namespace Material {
    ...
    void init()
    {
        ...
        for (int pt1 = NO_PIECE_TYPE; pt1 <= KING; ++pt1)
            for (int pt2 = NO_PIECE_TYPE; pt2 <= pt1; ++pt2)
            {
                if(pt1 == KNIGHT || pt2 == KNIGHT || pt1 == KING || pt2 == KING)
                {
...
Entry* probe(const Position& pos) {
  ...
  // in defining bishop pair bonuses.  Use KING as place holder for knight pair.
  const int pieceCount[COLOR_NB][PIECE_TYPE_NB] = {
  { pos.count<BISHOP>(WHITE) > 1, pos.count<PAWN>(WHITE), pos.count<KNIGHT>(WHITE),
    pos.count<BISHOP>(WHITE)    , pos.count<ROOK>(WHITE), pos.count<QUEEN >(WHITE),
    pos.count<KNIGHT>(WHITE) > 1 },
  { pos.count<BISHOP>(BLACK) > 1, pos.count<PAWN>(BLACK), pos.count<KNIGHT>(BLACK),
    pos.count<BISHOP>(BLACK)    , pos.count<ROOK>(BLACK), pos.count<QUEEN >(BLACK),
    pos.count<KNIGHT>(BLACK) > 1 } };
