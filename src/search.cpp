...
bool Search::Worker::iterative_deepening() {
    ...
    for (Color c : {WHITE, BLACK})
        for (int fromTh = 0; fromTh < 2; fromTh++)
            for (int toTh = 0; toTh < 2; toTh++)
                for (int i = 0; i < UINT_16_HISTORY_SIZE; i++)
                    mainHistory[c][fromTh][toTh][i] = mainHistory[c][fromTh][toTh][i] * 729 / 1024;
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, const bool cutNode) {
    ...
    if (((ss - 1)->currentMove).is_ok() && !(ss - 1)->inCheck && !priorCapture)
    {
        ...
        mainHistory[~us][bool((ss - 1)->threats & prevMove.from_sq())]
                   [bool((ss - 1)->threats & prevSq)][prevMove.raw()]
          << evalDiff * 11;
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            else if (!ss->followPV || !PvNode)
            {
                ...
                history += 69
                         * mainHistory[us][bool(ss->threats & move.from_sq())]
                                      [bool(ss->threats & move.to_sq())][move.raw()]
                         / 32;
    ...
    else if (!priorCapture && prevSq != SQ_NONE)
    {
        ...
        mainHistory[~us][bool((ss - 1)->threats & ((ss - 1)->currentMove).from_sq())]
                   [bool((ss - 1)->threats & prevSq)][((ss - 1)->currentMove).raw()]
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    Color us = pos.side_to_move();
    workerThread.mainHistory[us][bool(ss->threats & move.from_sq())]
                            [bool(ss->threats & move.to_sq())][move.raw()]
