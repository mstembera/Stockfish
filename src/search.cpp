...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    tte = TT.probe(posKey, ttHit, PvNode);
            ...
            &&  pos.rule50_count() == 0)
        {
            ...
            if (found)
            {
                ...
                tte->save(posKey, value_to_tt(value, ss->ply), BOUND_EXACT,
                          ...
                          MOVE_NONE, VALUE_NONE, TT.generation(), PvNode);
    ...
    else
    {
        ...
        tte->save(posKey, VALUE_NONE, BOUND_NONE, DEPTH_NONE, MOVE_NONE, ss->staticEval, TT.generation(), PvNode);
    }
...
moves_loop: // When in check and at SpNode search starts from here
    ...
    */
    ...
    tte->save(posKey, value_to_tt(bestValue, ss->ply),
              ...
              depth, bestMove, ss->staticEval, TT.generation(), PvNode);
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
        ...
        if (bestValue >= beta)
        {
            if (!ttHit)
                tte->save(pos.key(), value_to_tt(bestValue, ss->ply), BOUND_LOWER,
                          DEPTH_NONE, MOVE_NONE, ss->staticEval, TT.generation(), PvNode);
          ...
          if (value > alpha)
          {
              ...
              else // Fail high
              {
                  tte->save(posKey, value_to_tt(value, ss->ply), BOUND_LOWER,
                            ttDepth, move, ss->staticEval, TT.generation(), PvNode);
    ...
    tte->save(posKey, value_to_tt(bestValue, ss->ply),
              ...
              ttDepth, bestMove, ss->staticEval, TT.generation(), PvNode);
