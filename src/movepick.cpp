      ...
      if constexpr (Type == CAPTURES)
          ...
          m.value =      (*mainHistory)[pos.side_to_move()][from_to(m)]
                   ...
                   +     (threatened & from_sq(m) ?
                           ((type_of(pos.piece_on(from_sq(m))) == QUEEN  || type_of(pos.piece_on(from_sq(m))) == ROOK)   && !(to_sq(m) & threatenedByRook)  ? 50000
                          : (type_of(pos.piece_on(from_sq(m))) == KNIGHT || type_of(pos.piece_on(from_sq(m))) == BISHOP) && !(to_sq(m) & threatenedByMinor) ? 25000
