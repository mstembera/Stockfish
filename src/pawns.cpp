...
Score Entry::do_king_safety(const Position& pos, Square ksq) {
  ...
  Value bonus = shelter_storm<Us>(pos, ksq) + 4;

  // If we can castle use the bonus after the castling if it is bigger.
  if (!pos.did_castle(Us))
  {
      if (pos.can_castle(MakeCastling<Us, KING_SIDE>::right))
          bonus = std::max(bonus, shelter_storm<Us>(pos, relative_square(Us, SQ_G1)) - 2);

      if (pos.can_castle(MakeCastling<Us, QUEEN_SIDE>::right))
          bonus = std::max(bonus, shelter_storm<Us>(pos, relative_square(Us, SQ_C1)) - 2);
  }
  else // Bonus for having castled.
      bonus += 10;
